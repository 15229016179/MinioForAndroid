...
class Signer {
  ...
  private void setSigningKey() throws NoSuchAlgorithmException, InvalidKeyException, UnsupportedEncodingException {
    String aws4SecretKey = "AWS4" + this.secretKey;

    byte[] dateKey = sumHmac(aws4SecretKey.getBytes("UTF-8"),
                             this.date.toString(DateFormat.SIGNER_DATE_FORMAT).getBytes("UTF-8"));

    byte[] dateRegionKey = sumHmac(dateKey, this.region.getBytes("UTF-8"));

    byte[] dateRegionServiceKey = sumHmac(dateRegionKey, "s3".getBytes("UTF-8"));

    this.signingKey = sumHmac(dateRegionServiceKey, "aws4_request".getBytes("UTF-8"));
  }


  private void setSignature() throws NoSuchAlgorithmException, InvalidKeyException, UnsupportedEncodingException {
    byte[] digest = sumHmac(this.signingKey, this.stringToSign.getBytes("UTF-8"));
